...
#Imposes a range of 0.4 <-> 0.7 on the x-axis and -0.4 <-> 0.4 on the y-axis, to conform to Baxter's strange coordinate system.
newtok = [0,0,0,0]
current_pos = [0,0]
points = []
distances = []

file1 = open("andreeaFinalProcessedTableCloth.txt")
...
for line in file1:
	...
	xRange = float(5.0/6.0) * 0.3
	xOffset = 0.2#float((0.7 - xRange)/2)
	file3.write("("+tokens[0]+","+tokens[1]+")\n("+tokens[2]+","+tokens[3]+")\n")

	newtok[0] = (xRange*float(tokens[0]))/500 - 0.1# - 0.4
	newtok[1] = (0.3*float(tokens[1]))/600 + 0.4
	newtok[2] = (xRange*float(tokens[2]))/500 - 0.1# - 0.4
	newtok[3] = (0.3*float(tokens[3]))/600 + 0.4
